...
int main(int argc, char *argv[]){
	...
	version = read_version_srf08(SRF_HR);

	printf("Version 0x%x: %d\n", SRF_HR, version);

	measure_distance_srfx_cm(SRF_HL);
	measure_distance_srfx_cm(SRF_V);
	measure_distance_srfx_cm(SRF_HR);
	...
	lumen[2] = read_lumen_srfx(SRF_HR);
	...
	werte = read_all_data_srfx(SRF_HR, messwerte);
	...
	entfernung[2] = read_data_einheit_aan_srfx(SRF_HR);
