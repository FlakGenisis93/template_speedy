...
--!				26.04.2016 Kluge
--!
--! @details	- V1.1	Hinzufuegen von einem zweiten Eingabevektor damit 
--!							beide Motoren direkt angesprochen werden koennen.
--!				27.04.2016 Kluge
--!
--! @details	- V2.0	final version mit doku
--!				07.07.2016 Kluge
--!
--! @todo		---
--!
...
ENTITY uart_send_mm IS -- Sendeeinheit
	PORT(
		reset_n		: IN STD_LOGIC;							-- reset
		clk			: IN STD_LOGIC;							-- clock
		send_en		: IN STD_LOGIC := '0';					-- flag zum einhalten der baudrate
		send_active	: IN STD_LOGIC := '0';					-- flag zum starten der uebertragung
		send_m1_in	: IN STD_LOGIC_VECTOR(31 DOWNTO 0);	-- befehl fuer den motor1
		send_m2_in	: IN STD_LOGIC_VECTOR(31 DOWNTO 0); -- befehl fuer den motor2
		send_done	: OUT STD_LOGIC := '0';					-- flag zum erkennen der fertigen uebertragung
		send_out		: OUT STD_LOGIC := '0'					-- signalausgang zum sabertooth (UART RS232-C)
	);
END uart_send_mm;


ARCHITECTURE arch_uart_send_mm OF uart_send_mm IS
...
SIGNAL state_s : STATE_TYPE := idle_st;
	...
	--process der die kompletten Befehle empfaengt und sie direkt zum Sabertooth sendet. 
	--zeit: 400 usec fuer einen ganzen Befehl der aus zwei teilbefehlen besteht
	...
	BEGIN
		...
		ELSIF (clk'event and clk = '1') THEN
			...
			CASE state_s IS
				...
				WHEN idle_st =>							-- case idle: alle werte werden geloescht oder das senden wird vorbereitet
				...
				WHEN sending_st =>						-- case sending: es wird gesendet
					send_done <= '0';
					
					IF (send_en = '0') THEN 			-- kommt vom baud_gen modul, aktiviert das senden
						NULL;
						
					ELSE
						CASE sendPos_v IS									-- start senden des Befehls
					
							--erstes Paket fuer motor 1
							WHEN 0 => send_out <= '0';					-- 0 usec
							...
							WHEN 79 => send_out <= '1';				-- 400 usec ende der uebertragung
					...
					WHEN done_st =>	-- alles auf anfang und signalisierung ueber flag das die uebertragung zuende ist
						sendPos_v := 0;
						send_done <= '1';
						state_s <= idle_st;
