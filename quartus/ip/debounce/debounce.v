...
module debounce (
  ...
  parameter WIDTH_g = 32;         // set to be the width of the bus being debounced
  ...
  input wire [WIDTH_g-1:0] data_in;
  output wire [WIDTH_g-1:0] data_out;
  
  reg [TIMEOUT_WIDTH-1:0] counter [0:WIDTH_g-1];
  wire counter_reset [0:WIDTH_g-1];
  wire counter_enable [0:WIDTH_g-1];
  
  // need one counter per input to debounce
  genvar i;
  generate for (i = 0; i < WIDTH_g; i = i+1)
